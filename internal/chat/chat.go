...
func (m *Manager) Chat(ctx context.Context, user *store.User, prompt string) (string, error) {
	...
	answer := formatAnswer(resp.Choices[0].Message.Content)
...
// formatAnswer trims and normalizes model output for better readability in chat apps.
func formatAnswer(text string) string {
	...
	// Trim leading/trailing blank lines after collapsing.
